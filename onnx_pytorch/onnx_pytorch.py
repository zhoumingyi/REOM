...
class ConvertModel(nn.Module):
    def __init__(
        ...
        # print(get_outputs_names(onnx_model.graph))
    ...
    def forward(self, *input_list, **input_dict):
        ...
        for node in self.onnx_model.graph.node:
            ...
            for x in self.output_names:
                if x in activations:
                    # self.inter_outputs.append(activations[x])
                    # print(x)
                    self.inter_outputs[x] = activations[x].detach().clone()
                    self.output_names.remove(x)
            #----------------------------------------------------------------
            #----------------------------------------------------------------
        ...
        #----------------------------------------------------------------
        #my code
        outputs = {x: activations[x] for x in self.output_names}
        # outputs = [activations[x] for x in self.output_names]
        # outputs.update(self.output_names)
        # for i in range(len(self.inter_outputs)): 
            # outputs.append(self.inter_outputs[i])
            # outputs[]
        # print(self.inter_outputs)
        #----------------------------------------------------------------
        #----------------------------------------------------------------
        if len(outputs) == 1:
            outputs = outputs[0]
        if self.inter_name is not None:
            self.inter_outputs[self.inter_name] = inter_out
            # return self.inter_outputs
        if len(self.inter_outputs) == 1:
            value_iter = iter(self.inter_outputs)
            return self.inter_outputs[next(value_iter)]
        else: 
            return self.inter_outputs
        # if len(outputs) == 1:
        #     outputs = outputs[0]
